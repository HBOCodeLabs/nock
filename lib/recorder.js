...
function generateRequestAndResponse(body, options, res, datas, nockOptions, transformerNames, callback) {
  ...
  if (nockOptions.recordBodiesToFiles === true) {
  ...
  if (nockOptions.recordBodiesToFiles === true) {
    recordFiles.recordBodyToFile(options, requestBody, responseBody, nockOptions, function(err, filename) {
      if (err) {
...
function record(dont_print, nockOptions) {
  [http, https].forEach(function(module) {
    ...
    module.request = function(options, callback) {
      ...
      var shouldRecord = recordFilter(options, nockOptions);
      ...
      req = oldRequest.call(http, options, function(res) {
        ...
        if (shouldRecord) {
          ...
          // Apply transforms
          var recordedResponse = transformer.transformRecordedResponse(res, nockOptions, transformerNames);
          ...
          recordedResponse.once('end', function() {
            generateRequestAndResponse(
              body, options, recordedResponse, datas, nockOptions, transformerNames, function(out) {
                outputs.push(out);
                if (! dont_print) { console.log(SEPARATOR + out + SEPARATOR); }
                events.emitNockEvent('onResponseComplete', options);
            });
          });
        }
