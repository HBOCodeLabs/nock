	...
	if (res.headers) {
	  ...
	  for (var headerName in res.headers) {
	    ...
	    case 'content-length':
	      ...
	      break;
	    default:
	      newHeaders[headerName] = res.headers[headerName];
	    }
	  }
	}
	generateRequestAndResponseDecoded(requestBody, body.toString(), options, res, newHeaders, recordOptions, callback);
      }
    });
  } else {
    // Not unzipping...
    generateRequestAndResponseDecoded(requestBody, responseBody, options, res, res.headers, recordOptions, callback);
  }
}
...
function generateRequestAndResponseDecoded(requestBody, responseBody, options, res, responseHeaders, recordOptions, callback) {
  ...
  if (recordOptions.reformatJSON === true) {
    if (responseHeaders['content-type'].search('application/json') >= 0) {
