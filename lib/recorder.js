...
function generateRequestAndResponse(body, options, res, datas, recordOptions, callback) {
  ...
  var responseHeaders = _.clone(res.headers);
    ...
    module.request = function(options, callback) {
      ...
      req = oldRequest.call(http, options, function(res) {
        ...
        if (shouldRecord) {
          ...
          var responseTransformers = recordOptions.responseTransformers;
          var transformedResponse = res;
          if (responseTransformers) {
            for (var i = 0; i < responseTransformers.length; i++) {
              transform = responseTransformers[i].transformRecord(transformedResponse.headers, transformedResponse);
              if (transform) {
                transformedResponse = transform;
              }
            }
          }

          var datas = [];
          transformedResponse.on('data', function(data) {
            datas.push(data);
          });

          if (module === https) { options._https_ = true; }

          transformedResponse.once('end', function() {
            var out = generateRequestAndResponse(body, options, transformedResponse, datas, recordOptions, function(out) {
              outputs.push(out);
