...
var RecordedResponse = require('./recordedResponse');
  ...
  [http, https].forEach(function(module) {
    ...
    module.request = function(options, callback) {
      ...
      req = oldRequest.call(http, options, function(res) {
        ...
        if (shouldRecord) {
          var recordedResponse = new RecordedResponse(res);
          ...
          if (responseTransformers) {
            for (var i = 0; i < responseTransformers.length; i++) {
              var transformedResponse = responseTransformers[i].transformRecord(recordedResponse);
              if (transformedResponse) {
                recordedResponse = transformedResponse;
                transformerNames.push( responseTransformers[i].name );
              }
            }
          }
