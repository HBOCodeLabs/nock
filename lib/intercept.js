var RequestOverrider = require('./request_overrider'),
    ...
    shouldRecord     = require('./recordFilter'),
    util             = require('util');
      ...
      module.request = function(options, callback) {
        ...
        if (isOn()) {
          if (interceptors.length) {
            ...
            // We have no interceptors.  Would we have recorded this call?
            if (shouldRecord(options, nockOptions)) {
              // We would have recorded this call, but it's not expected.
              // Throw an error
              throw new Error('Nock: No match for ' + proto + ' request: ' + util.inspect(options));
            }
          }
        }
        return oldRequest.apply(module, arguments);
