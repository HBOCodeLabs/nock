  ...
  function intercept(uri, method, requestBody, interceptorOptions) {
    ...
    function reply(statusCode, body, headers, responseTransformers) {
      ...
      if (responseTransformers) {
        for (var i = (responseTransformers.length - 1); i >= 0; i--) {
          body = responseTransformers[i].transformPlayback(headers, body);
        }
      }
    ...
    function replyWithFile(statusCode, filePath, headers, responseTransformer) {
