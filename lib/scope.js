...
function startScope(basePath, options) {
  ...
  function intercept(uri, method, requestBody, interceptorOptions) {
    ...
    function responseError(error) {
      this.options = interceptorOptions || {};
      this.responseErrorVal = error;
      add(key, this, scope);
      return scope;
    }
    
    function reply(statusCode, body, headers, responseTransformers) {
      ...
      if (responseTransformers) {
        var transformedResponse = transformer.transformPlaybackResponse( headers, body, responseTransformers );
    ...
    function replyWithFile(statusCode, filePath, headers, responseTransformers) {
      var readStream = fs.createReadStream(filePath);
      readStream.pause();
      return reply.call(this, statusCode, readStream, headers, responseTransformers);
    }
    ...
    function match(options, body) {
      ...
      var matchKey = method.toUpperCase() + ' ' + utilities.generateHostUrlString(options);
      matchKey += path;
    ...
    function matchIndependentOfBody(options) {
      ...
      var matchKey = method.toUpperCase() + ' ' + utilities.generateHostUrlString(options) + path;
  ...
  function transformResponse(responseTransformer) {
