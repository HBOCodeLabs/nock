...
function stringifyRequest(options, body) {
  ...
  return method + ' ' + utilities.generateHostUrlString(options) + path + ' ' + body;
}
...
function RequestOverrider(req, options, interceptors, remove) {
  ...
  req.abort = function() {
    ...
    delay = 0;
    if (interceptor.responseDelay) {
      delay = interceptor.responseDelay;
    }

    remove(interceptor);
    interceptor.discard();

    if (interceptor.requestErrorVal) {
      setTimeout(function() {
        req.emit('error', interceptor.requestErrorVal);
      }, delay);
      return;
    }

    if (interceptor.responseErrorVal) {
      setTimeout(function() {
        response.emit('error', interceptor.responseErrorVal);
      }, delay);
      return;
    }
